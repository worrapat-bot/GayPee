...
// ต้องมี Component เหล่านี้ทั้งหมดใน GameObject เดียวกัน
[RequireComponent(typeof(FieldOfView))]
[RequireComponent(typeof(NavMeshAgent))]
[RequireComponent(typeof(Animator))]
[RequireComponent(typeof(AudioSource))]
[RequireComponent(typeof(NavMeshObstacle))] // ⬅️ ตัว Obstacle
public class EnemyPatrol : MonoBehaviour
{
    ...
    [SerializeField] protected string nextSceneName = "JumpScareScene"; // ⬅️ ฉากที่จะโหลด
    ...
    protected NavMeshObstacle obstacle; // ⬅️ ตัวแปร Obstacle
    ...
    protected virtual void Start()
    {
        ...
        // 💡 ตั้งค่าเริ่มต้น: Agent เดิน (เปิด) Obstacle บล็อก (ปิด)
        SwitchToObstacle(false);
    ...
    // ⬇️ ฟังก์ชันสำหรับสลับ Agent (เดิน) กับ Obstacle (หยุดนิ่ง)
    protected void SwitchToObstacle(bool isObstacle)
    {
        if (agent != null) agent.enabled = !isObstacle;
        if (obstacle != null) obstacle.enabled = isObstacle;
    }
    ...
    protected virtual void Update()
    {
        ...
        if (currentlyDetecting)
        {
            ...
            SwitchToObstacle(false); // 💡 ไล่ล่า = ใช้ Agent
        ...
        {
            SwitchToObstacle(false); // 💡 Patrol/Search = ใช้ Agent
    ...
    protected void HandleFootsteps()
    {
        ...
        if (agent == null || !agent.enabled || agent.isStopped) return;
    ...
    protected void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player") && !isGameOverSequenceActive)
        {
            ...
            SwitchToObstacle(true); // 💡 ชน = ใช้ Obstacle
            HandleJumpscare();
        }
    }
    ...
    {
        PlaySFX(searchClip);
        SwitchToObstacle(false); // 💡 ไปค้นหา = ใช้ Agent
        ...
        agent.isStopped = true;
        SwitchToObstacle(true); // 💡 หยุดค้นหา = ใช้ Obstacle
        yield return StartCoroutine(SearchRoutine());
        lastKnownPlayerPosition = null;
        isCheckingLastKnownPosition = false;
        GoToNextPoint();
        
        SwitchToObstacle(false); // 💡 กลับไป Patrol = ใช้ Agent
        agent.speed = moveSpeed;
        agent.isStopped = false;
        agent.SetDestination(patrolPoints[currentPointIndex].position);
    }
    ...
    protected IEnumerator SearchRoutine()
    {
        ...
        SwitchToObstacle(true); // 💡 หมุนค้นหา = ใช้ Obstacle
    ...
    protected virtual void PatrolMovement()
    {
        ...
        SwitchToObstacle(false); // 💡 Patrol = ใช้ Agent
        agent.speed = moveSpeed;
        agent.stoppingDistance = 0.05f;
        ...
        if (!agent.pathPending && agent.remainingDistance < agent.stoppingDistance && !isSearching)
        {
            ...
            SwitchToObstacle(true); // 💡 หยุดรอ = ใช้ Obstacle
    ...
    protected void RotateTowardsTarget(Vector3 targetPos)
    {
        ...
        if (direction.sqrMagnitude > 0.01f)
        {
            ...
            StartCoroutine(RotateToTarget(targetRotation, rotationSpeed));
        }
    }
}
